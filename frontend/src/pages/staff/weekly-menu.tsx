...
export function StaffWeeklyMenu() {
  const { email } = useAuth()
  ...
  return (
    <PageTransition>
      <div className="space-y-6">
        <PageHeader title="Weekly Menu" description="Menu for the coming week" icon={Calendar} />
                        ...
                        {MEAL_TYPES.map((mealType) => {
                          ...
                          return (
                            <div key={mealType} className="rounded-lg border p-3">
                              <span className="text-sm font-medium capitalize mb-2 block">{mealType}</span>
                              {mealItems.length > 0 ? (
                                  ...
                                  {mealItems.map((item) => (
                                    <div key={item.ScheduleID} className="flex items-center justify-between">
                                      <div className="flex items-center gap-1.5 min-w-0">
                                        ...
                                        {item.IsVegetarian && (
                                          <Badge variant="secondary" className="text-[9px] px-1 py-0">Veg</Badge>
                                        )}
                                      </div>
