...
import { CalendarCheck, Star, CookingPot } from 'lucide-react'

export function StaffTodayMenu() {
  const { email } = useAuth()
  ...
  return (
    <PageTransition>
      <div className="space-y-6 max-w-4xl mx-auto">
        <PageHeader title="Today's Menu" description={formatDate(today ?? new Date())} icon={CalendarCheck} />
                ...
                <Card key={mealType} className="overflow-hidden">
                  ...
                  <CardContent className="pt-4">
                    <div className="space-y-3">
                      {items.map((item) => (
                        <div key={item.ScheduleID} className="flex items-center justify-between rounded-lg border p-3 transition-colors hover:bg-muted/50">
                          <div className="flex items-center gap-3 min-w-0">
                            <div>
                              <div className="flex items-center gap-2">
                                <span className="font-medium">{item.ItemName}</span>
                                {item.IsVegetarian && (
                                  <Badge variant="secondary" className="text-[10px] px-1.5 py-0">Veg</Badge>
                                )}
                              </div>
