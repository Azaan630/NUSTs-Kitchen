...
-- Add Subject column if missing (safe on tables created with old schema)
SET @hasSubject = (SELECT COUNT(*) FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'Feedback' AND COLUMN_NAME = 'Subject');
SET @addCol = IF(@hasSubject = 0, 'ALTER TABLE Feedback ADD COLUMN Subject VARCHAR(200) NOT NULL DEFAULT '''' AFTER Email', 'SELECT 1');
PREPARE st FROM @addCol; EXECUTE st; DEALLOCATE PREPARE st;
...
(4,DATE_SUB(CURDATE(),INTERVAL 10 DAY), DATE_SUB(CURDATE(),INTERVAL 7 DAY),  'Approved'),
(5,DATE_ADD(CURDATE(),INTERVAL 15 DAY), DATE_ADD(CURDATE(),INTERVAL 18 DAY), 'Cancelled'),
(7,DATE_SUB(CURDATE(),INTERVAL 8 DAY),  DATE_SUB(CURDATE(),INTERVAL 6 DAY),  'Rejected'),
(8,DATE_ADD(CURDATE(),INTERVAL 20 DAY), DATE_ADD(CURDATE(),INTERVAL 25 DAY), 'Pending'),
(9,DATE_SUB(CURDATE(),INTERVAL 3 DAY),  DATE_ADD(CURDATE(),INTERVAL 2 DAY),  'Cancelled');
...
('Aisha Khan',  'aisha.student@seecs.edu.pk',  'More Biryani Please!',       'The Chicken Biryani on Tuesday was amazing! Please add it more often.'),
('Bilal Ahmed', 'bilal.student@seecs.edu.pk',  'Suggestion: Add Pasta',     'Can we have a suggestion box for new dishes? Would love to see Pasta on the menu.'),
('Zara Malik',  'zara.student@seecs.edu.pk',   'Mess-Off System is Great',   'The mess-off request process is super smooth. Appreciate the quick approvals!'),
('Hassan Raza', 'hassan.student@seecs.edu.pk', 'Dark Mode Readability',     'Dark mode looks gorgeous but some text is hard to read on mobile.'),
('Sara Javed',  'sara.student@seecs.edu.pk',   'Billing Date Format',       'Billing tab shows my old bills correctly but the Due Date format is confusing.');
