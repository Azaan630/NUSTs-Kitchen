...
async def api_reject_mess_off(email, rid): return await _req("DELETE", f"/student/mess_off/cancel/{rid}", {"email": email})
...
async def api_get_poll_results(email): return await _req("GET", "/admin/poll/results", {"email": email})
...
class AdminPage:
    ...
    RENDERERS = [
        "_render_students", "_render_staff", "_render_food",
        "_render_mess_off", "_render_bills", "_render_menu", "_render_polls",
    ]
    ...
    def _field(self, label, hint=""):
        return ft.TextField(
            ...
            label_style=ft.TextStyle(color=self.P.MUTED, font_family="DM Sans"),
            text_style=ft.TextStyle(color=self.P.TEXT_LIGHT, font_family="DM Sans"),
            border_radius=10, filled=True, fill_color=self.P.SURFACE,
            cursor_color=self.P.PRIMARY,
        )
    ...
    def _fab(self, label, icon, cb):
        return ft.Container(
            content=ft.FilledButton(
                content=ft.Row([
                    ft.Icon(icon, size=16, color=self.P.WHITE),
                    ft.Text(label, size=12, weight="bold", color=self.P.WHITE, font_family="DM Sans"),
    ...
    def _form_card(self, title, fields, btn):
        return ft.Container(
            content=ft.Column([
                ft.Row([
                    ft.Icon(ft.Icons.ADD_CIRCLE_OUTLINE_ROUNDED, size=18, color=self.P.PRIMARY_LT),
                    self._title(title),
                ], spacing=8),
                ft.Container(height=8),
                ft.Column(fields, spacing=8),
                ft.Container(height=4),
                ft.Row([btn], alignment=ft.MainAxisAlignment.END),
            ], spacing=0),
            bgcolor=ft.Colors.with_opacity(0.05, self.P.WHITE), border_radius=16,
            padding=ft.Padding.symmetric(horizontal=20, vertical=16),
            margin=ft.Margin.only(bottom=16),
            border=ft.Border.all(0.5, ft.Colors.with_opacity(0.08, self.P.WHITE)),
        )
    ...
    async def _render_students(self, content_ref):
        ...
        form = self._form_card("Register New Student", [
        ...
        ], self._fab("Register", ft.Icons.PERSON_ADD_ROUNDED, add_student))
        ...
        for s in students:
            ...
            rows.append(self._row_card([
                ft.Container(
                    content=ft.Text(
                        f"{s.get('First_Name','')} {s.get('Last_Name','')}"[:2].upper(),
                        size=14, weight="bold", color=self.P.WHITE,
                    ),
                    width=40, height=40,
                    gradient=ft.LinearGradient(
                        begin=ft.Alignment(-1, -1), end=ft.Alignment(1, 1),
                ...
                ft.Column([
                    ft.Text(f"{s.get('First_Name','')} {s.get('Last_Name','')}".strip(),
                            size=13, weight="bold", color=self.P.TEXT_LIGHT, font_family="DM Sans"),
                    ft.Text(s.get("Email",""), size=11, color=self.P.MUTED, font_family="DM Sans"),
                ], expand=True, spacing=2),
                ft.Text(f"ID #{uid}", size=11, color=self.P.MUTED, font_family="DM Sans"),
            ], actions=[
                self._icon_btn(ft.Icons.DELETE_ROUNDED, self.P.ERROR, "Delete", do_del),
            ]))

        list_section = ft.Column([
            ft.Row([
                self._title(f"All Students ({len(students)})"),
            ...
            ft.Text("No students found.", color=self.P.MUTED, font_family="DM Sans"),
        ])

        content_ref.content = ft.Column([form, list_section], scroll=ft.ScrollMode.ADAPTIVE, expand=True)
        self.page.update()
    ...
    async def _render_staff(self, content_ref):
        ...
        form = self._form_card("Register New Staff", [
            ft.Row([f_fn, f_ln, f_em, f_cat], spacing=8),
        ], self._fab("Register", ft.Icons.PERSON_ADD_ROUNDED, add_staff))
        ...
        list_section = ft.Column([
            ft.Row([
                self._title(f"All Staff ({len(staff_list)})"),
                self._icon_btn(ft.Icons.REFRESH_ROUNDED, self.P.MUTED, "Refresh",
                               lambda e: asyncio.create_task(self._render_staff(content_ref))),
            ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
            ft.Container(height=6),
            ft.Column(rows, scroll=ft.ScrollMode.ADAPTIVE) if rows else
            ft.Text("No staff found.", color=self.P.MUTED, font_family="DM Sans"),
        ])
    ...
    async def _render_food(self, content_ref):
        ...
        form = self._form_card("Add Food Item", [
            ft.Row([f_name, f_qty, f_price], spacing=8),
        ], self._fab("Add Item", ft.Icons.ADD_ROUNDED, add_food))
        ...
        for item in items:
            ...
            ef_name  = ft.TextField(value=item.get("Name",""), expand=True,
                                    border_color=self.P.PRIMARY, border_radius=8,
                                    text_style=ft.TextStyle(color=self.P.TEXT_LIGHT, font_family="DM Sans"),
                                    filled=True, fill_color=self.P.SURFACE, dense=True, cursor_color=self.P.PRIMARY)
            ...
            rows.append(self._row_card([
                ...
                ft.Column([
                    ef_name,
                    ft.Text(f"Cost: PKR {cost:.0f}", size=11, color=self.P.MUTED, font_family="DM Sans"),
                ], expand=True, spacing=4),
                ef_price,
            ], actions=[
                self._icon_btn(ft.Icons.SAVE_ROUNDED, self.P.PRIMARY_LT, "Save", do_update),
                self._icon_btn(ft.Icons.DELETE_ROUNDED, self.P.ERROR, "Delete", do_del),
            ]))
    ...
    async def _render_mess_off(self, content_ref):
        ...
        STATUS_COLORS = {
            ...
            "Cancelled": (self.P.MUTED, ft.Colors.with_opacity(0.05, self.P.WHITE)),
        }
        ...
        for req in requests:
            ...
            fg, bg = STATUS_COLORS.get(status, (self.P.MUTED, ft.Colors.with_opacity(0.05, self.P.WHITE)))
            ...
            if status == "Pending":
                ...
                actions = [
                    self._icon_btn(ft.Icons.CHECK_CIRCLE_ROUNDED, self.P.SUCCESS, "Approve", do_approve),
                    self._icon_btn(ft.Icons.CANCEL_ROUNDED, self.P.ERROR, "Reject", do_reject),
                ]
        ...
        content_ref.content = ft.Column([
            ft.Row([
                self._title(f"Mess-Off Requests ({len(requests)})"),
                self._icon_btn(ft.Icons.REFRESH_ROUNDED, self.P.MUTED, "Refresh",
                               lambda e: asyncio.create_task(self._render_mess_off(content_ref))),
            ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
            ft.Container(height=8),
            ft.Column(rows, scroll=ft.ScrollMode.ADAPTIVE) if rows else
            ft.Text("No requests found.", color=self.P.MUTED, font_family="DM Sans"),
    ...
    async def _render_bills(self, content_ref):
        ...
        form = self._form_card("Add Bill", [
            ft.Row([f_uid, f_amount, f_due, f_month], spacing=8),
        ], self._fab("Create Bill", ft.Icons.ADD_ROUNDED, add_bill))
        ...
        for b in bills:
            ...
            rows.append(self._row_card([
                ft.Container(
                    content=ft.Icon(ft.Icons.RECEIPT_ROUNDED, size=20, color=self.P.PRIMARY_LT),
                    width=40, height=40,
                    bgcolor=ft.Colors.with_opacity(0.12, self.P.PRIMARY),
                    border_radius=12, alignment=ft.Alignment(0, 0),
                ),
        ...
        list_section = ft.Column([
            ft.Row([
                self._title(f"Monthly Summary ({len(bills)} entries)"),
                self._icon_btn(ft.Icons.REFRESH_ROUNDED, self.P.MUTED, "Refresh",
                               lambda e: asyncio.create_task(self._render_bills(content_ref))),
            ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
            ft.Container(height=6),
            ft.Column(rows, scroll=ft.ScrollMode.ADAPTIVE) if rows else
            ft.Text("No billing data.", color=self.P.MUTED, font_family="DM Sans"),
        ])
    ...
    async def _render_menu(self, content_ref):
        ...
        async def add_to_menu(e):
            res = await api_add_menu_item(
                self.email, f_item_id.value.strip(), f_date.value.strip(), f_meal_type.value.strip(),
            )
            ok = "error" not in (res or {})
            self._snack("Added to menu!" if ok else res.get("error","Error"), ok)
            if ok: await self._render_menu(content_ref)

        form = self._form_card("Add Item to Menu", [
            ft.Row([f_item_id, f_date, f_meal_type], spacing=8),
        ], self._fab("Add to Menu", ft.Icons.ADD_ROUNDED, add_to_menu))

        MEAL_COLORS = {"Breakfast": self.P.WARNING, "Lunch": self.P.SUCCESS, "Dinner": self.P.SECONDARY}
        ...
        for item in items:
            ...
            rows.append(self._row_card([
                ft.Container(
                    ...
                    width=30, height=30,
                    bgcolor=ft.Colors.with_opacity(0.15, mc),
                    border_radius=8, alignment=ft.Alignment(0, 0),
                ),
                ft.Column([
                    ft.Text(name, size=13, weight="bold", color=self.P.TEXT_LIGHT, font_family="DM Sans"),
                    ft.Text(f"{d}  •  {mt}", size=11, color=self.P.MUTED, font_family="DM Sans"),
                ], expand=True, spacing=2),
                ft.Text(f"Sched #{sid}", size=10, color=self.P.MUTED, font_family="DM Sans"),
            ], actions=[
                self._icon_btn(ft.Icons.REMOVE_CIRCLE_ROUNDED, self.P.ERROR, "Remove", do_del),
            ]))

        list_section = ft.Column([
            ft.Row([
                self._title(f"Weekly Menu ({len(rows)} slots)"),
                self._icon_btn(ft.Icons.REFRESH_ROUNDED, self.P.MUTED, "Refresh",
                               lambda e: asyncio.create_task(self._render_menu(content_ref))),
            ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
            ft.Container(height=6),
            ft.Column(rows, scroll=ft.ScrollMode.ADAPTIVE) if rows else
            ft.Text("No menu items.", color=self.P.MUTED, font_family="DM Sans"),
        ])
    ...
    async def _render_polls(self, content_ref):
        ...
        for r in poll_results:
            result_cards.append(self._row_card([
                ft.Container(
                    content=ft.Icon(ft.Icons.HOW_TO_VOTE_ROUNDED, size=20, color=self.P.PRIMARY_LT),
                    width=40, height=40,
                    bgcolor=ft.Colors.with_opacity(0.12, self.P.PRIMARY),
                    border_radius=12, alignment=ft.Alignment(0, 0),
                ),
                ft.Text(r.get("Name", "?"), expand=True, weight="bold",
                        color=self.P.TEXT_LIGHT, font_family="DM Sans"),
                ft.Column([
                    ft.Text(f"{r.get('Vote_Count', 0)}", size=20, weight="bold",
                            color=self.P.PRIMARY_LT, font_family="DM Sans"),
                    ft.Text("votes", size=10, color=self.P.MUTED, font_family="DM Sans"),
        ...
        async def launch_poll(e):
            try:
                ...
                if ok: await self._render_polls(content_ref)
            except:
                self._snack("IDs must be numbers separated by commas", False)

        new_poll_form = self._form_card("Launch New Poll", [
            ft.Text("This will replace the current active poll.", size=12, color=self.P.MUTED),
            ft.Row([f_meal, f_ids], spacing=8),
        ], self._fab("Start Poll", ft.Icons.HOW_TO_VOTE, launch_poll))

        content_ref.content = ft.Column([
            ft.Row([
                self._title("Live Poll Standings"),
                self._icon_btn(ft.Icons.REFRESH_ROUNDED, self.P.MUTED, "Refresh",
                               lambda e: asyncio.create_task(self._render_polls(content_ref))),
            ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
            ft.Container(height=8),
            ft.Column(result_cards) if result_cards else
            ft.Container(
                content=ft.Column([
                    ft.Icon(ft.Icons.HOURGLASS_EMPTY_ROUNDED, size=40, color=self.P.MUTED),
                    ft.Text("No active poll", color=self.P.MUTED, font_family="DM Sans"),
