...
class StudentHomePage:
    ...
    def _item_name(self, item: dict) -> str:
        return (
            item.get("Food_Item_Name") or item.get("Name")
            or item.get("Item_Name") or item.get("item_name")
    ...
    def _item_meal_type(self, item: dict) -> str:
        ...
        return mt.capitalize() if mt else "Meal"

    def _item_rating(self, item: dict):
        val = item.get("Ratings_Average") or item.get("Rating") or item.get("Avg_Rating") or 0
        try:
            return float(val)
    ...
    def _loading(self):
        return ft.Container(
            content=ft.Column([
                ...
                ft.Text("Fetching menu...", color=self.sub_c, font_family="DM Sans", size=14),
    ...
    def _error(self, msg):
        return ft.Container(
            content=ft.Column([
                ft.Icon(ft.Icons.ERROR_OUTLINE_ROUNDED, color=self.P.ERROR, size=48),
                ft.Text(msg, color=self.P.ERROR, size=14, font_family="DM Sans", text_align=ft.TextAlign.CENTER),
    ...
    def _meal_color(self, meal_type: str):
        ...
        if "breakfast" in mt: return self.P.WARNING
        if "lunch"     in mt: return self.P.SUCCESS
        if "dinner"    in mt: return self.P.SECONDARY
        return self.P.PRIMARY_LT
    ...
    def _build_stars(self, item_id, schedule_id, existing_rating, meal_date, meal_type):
        ...
        def render_stars(score, locked=False):
            for i, s in enumerate(star_refs):
                ...
                s.color = self.P.WARNING if filled else self.sub_c
            self.page.update()
        ...
        async def on_star_click(e, score):
            ...
            result = await rate_food_item(
                ...
                meal_date=meal_date, meal_type=meal_type, score=score, email=self.email,
            )
            if result and "error" not in result:
                rating_state["submitted"] = True
                self.page.snack_bar = ft.SnackBar(
                    content=ft.Text("Rating saved!", color="#FFF"), bgcolor=self.P.SUCCESS,
                )
        ...
        for i in range(1, 6):
            ...
            star_icon = ft.Icon(
                ...
                size=18, color=self.P.WARNING if filled else self.sub_c,
            )
            ...
            star_btn = ft.Container(
                ...
                on_hover=lambda e, s=score: (
                    asyncio.create_task(on_star_hover(e, s)) if e.data == "true"
        ...
        locked_label = ft.Text(
            ...
            size=11, color=self.P.WARNING if rating_state["submitted"] else self.sub_c,
            font_family="DM Sans",
        )
    ...
    def _build_food_card(self, item, meal_date=None, show_date=False):
        ...
        avg_stars = ft.Row([
            ...
            ft.Text(f"{avg:.1f}", size=12, color=self.sub_c, font_family="DM Sans"),
        ], spacing=2)
        ...
        star_row = self._build_stars(
            ...
            schedule_id=schedule_id, existing_rating=existing_rating,
            meal_date=date_for_rating, meal_type=meal_type_for_rating,
        )
        ...
        if show_date:
            d = item.get("Date", "")
            date_badge = ft.Container(
                content=ft.Text(str(d), size=10, color=self.sub_c, font_family="DM Sans"),
                bgcolor=ft.Colors.with_opacity(0.1, self.P.WARNING),
        ...
        card = ft.Container(
            content=ft.Column([
                ft.Row([
                    ...
                    ft.Column([
                        ft.Text(name, size=14, weight="bold", color=self.txt_c,
                                font_family="DM Sans", max_lines=1, overflow=ft.TextOverflow.ELLIPSIS),
                        avg_stars,
    ...
    def _build_today(self, menu_data):
        ...
        if not menu_items:
            return ft.Container(
                content=ft.Column([
                    ft.Icon(ft.Icons.NO_MEALS_ROUNDED, size=48, color=self.sub_c),
                    ft.Text("No menu for today", color=self.sub_c, font_family="DM Sans"),
                ], horizontal_alignment=ft.CrossAxisAlignment.CENTER, spacing=12),
                alignment=ft.Alignment(0, 0), padding=60,
            )
        ...
        for meal_type, items in groups.items():
            ...
            header = ft.Container(
                content=ft.Row([
                    ...
                    ft.Text(meal_type, size=16, weight="bold", color=color, font_family="DM Sans"),
                ], spacing=8),
        ...
        return ft.Container(
            content=ft.Column([
                ft.Row([
                    ft.Icon(ft.Icons.CALENDAR_TODAY_ROUNDED, color=self.P.PRIMARY_LT, size=18),
                    ft.Text(f"Today - {date_str}", size=15, color=self.sub_c, font_family="DM Sans"),
                ], spacing=8),
    ...
    def _build_weekly(self, items):
        if not items:
            return ft.Container(
                content=ft.Column([
                    ft.Icon(ft.Icons.CALENDAR_VIEW_WEEK_ROUNDED, size=48, color=self.sub_c),
                    ft.Text("No weekly menu available", color=self.sub_c, font_family="DM Sans"),
                ], horizontal_alignment=ft.CrossAxisAlignment.CENTER, spacing=12),
                alignment=ft.Alignment(0, 0), padding=60,
            )
        ...
        for d, meals in date_groups.items():
            ...
            day_header = ft.Container(
                content=ft.Row([
                    ft.Container(
                        content=ft.Text(weekday[:3].upper(), size=11, weight="bold",
                                        color=self.card_bg if is_today else self.P.PRIMARY_LT,
                                        font_family="DM Sans"),
                        bgcolor=self.P.PRIMARY if is_today else ft.Colors.TRANSPARENT,
                        border_radius=8, padding=ft.Padding.symmetric(horizontal=10, vertical=4),
                    ),
                    ft.Text(d, size=13, color=self.sub_c, font_family="DM Sans"),
                    ft.Container(
                        content=ft.Text("Today", size=10, color=self.P.PRIMARY_LT, font_family="DM Sans"),
                        visible=is_today,
            ...
            for mt, fitems in meals.items():
                ...
                for item in fitems:
                    sections.append(self._build_food_card(item, meal_date=meal_date, show_date=False))
    ...
    def _toggle_pill(self, on_today, on_weekly):
        today_btn = ft.Container(
            ...
            border_radius=20,
            bgcolor=self.P.PRIMARY if not self._view["weekly"] else ft.Colors.TRANSPARENT,
        ...
        week_btn = ft.Container(
            ...
            padding=ft.Padding.symmetric(horizontal=20, vertical=8),
            border_radius=20,
            bgcolor=self.P.PRIMARY if self._view["weekly"] else ft.Colors.TRANSPARENT,
            on_click=lambda e: on_weekly(),
        )
        today_btn.content.color = self.card_bg if not self._view["weekly"] else self.sub_c
        week_btn.content.color  = self.card_bg if self._view["weekly"] else self.sub_c

        return ft.Container(
            content=ft.Row([today_btn, week_btn], spacing=4),
            bgcolor=ft.Colors.with_opacity(0.08, self.P.WHITE),
            border_radius=24, padding=ft.Padding.all(4),
        )
    ...
    async def _render(self):
        ...
        self.main_container.content = ft.Column([
            ft.Row([
                ft.Text("Menu", size=28, weight="bold", font_family="DM Sans", color=self.txt_c),
                pill,
                ft.IconButton(
